...
{-filter' f (h :. t)= foldRight (\h -> bool id ((:.) h) (f h) Nil-}
...
{-(++) (x :. xs) y = x :. (xs ++ y)-}
{-!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!-}
{-x ++ y = foldRight undefined _undefined x-}
{-x ++ y = foldRight undefined y x-}
...
{-seqOptional (h :. t) = undefined-}
  {-bindOpt h (\a ->-}
  {-bindOpt (seqOptional t) (\list -> Full (a :. list)))-}
{-bindOpt = flip bindOptional-}

-- List (t a) -> t (Optional a)
-- As long as t supports some notion of `bind` and `Full`
-- This is a monad (something that has a bind and full analogy)
...
-- Move function call into a tail position
